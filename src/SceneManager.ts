...
export class SceneManager {
	...
	public addProduct() {
		...
		return new WizardScene("addProduct",
			async ctx => {
				await ctx.reply("Введите название товара", Markup.removeKeyboard().extra());
	...
	public checkout() {
		...
		checkout.enter(async ctx => {
			...
			if (content) {
				...
				ctx.reply("Такого заказа не существует⬛")
			}
		})
		return checkout;
	}
	...
	public final() {
		...
		final.enter(async ctx => {
			await ctx.reply("Поздравляем! Заказ был успешно оплачен.✅");
			...
			await ctx.telegram.sendMessage(admin_chat_id, content);
